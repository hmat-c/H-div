...
sub help {
    ...
    hmat-assign.pl: Read <inputfile> or input files whose names are prefixed by <prefix> and generate a PDF file that illustrates how leaf matrices in an H-matrix are assigned to MPI processes and worker threads.
...
my $Fname_eps = ($Inputfile0)?"$Inputfile0.pdf":"$Prefix.pdf";
...
print_gp 'set terminal pdfcairo enhanced color'."\n";
...
print "* * * Summary * * *\n";
if ( -e $Fname_eps ) {
    print "An PDF file $Fname_eps generated.\n";
}
